...
import { migrateDataFile, publishDataFile } from "./data-paths";
...
function assertNoLegacyService(
  ...
  if (existsSync(legacy.definition)) refuse();
  if (platform === "windows-task") {
    const query = queryWindowsTask(windowsTaskName(legacy), runCommand);
    if (query.status === "error")
      throw new Error(
        `cannot check legacy scheduled task (exit ${query.code}); no service changes made`,
      );
    if (query.status === "exists") refuse();
  } else if (platform === "systemd") {
    const code = runCommand(
      ["systemctl", "--user", "--no-pager", "status", "herdr-gui.service"],
      { quiet: true },
    );
    if (code === 0 || code === 3) refuse();
    if (code !== 4)
      throw new Error(
        `cannot check legacy systemd service (exit ${code}); no service changes made`,
      );
  } else {
    ...
    const code = runCommand(
      ["launchctl", "print", `gui/${runtime.uid}/dev.herdr.herdr-gui`],
      { quiet: true },
    );
    if (code === 0) refuse();
    if (code !== LAUNCHD_SERVICE_NOT_FOUND_EXIT_CODE)
      throw new Error(
        `cannot check legacy launchd service (exit ${code}); no service changes made`,
      );
  }
}
...
function runServiceAction(
  ...
  if (action === "reload") {
    ...
    if (platform === "systemd") {
      ...
      return runCommand(["systemctl", "--user", "restart", "roamgate.service"]);
    }
    if (platform === "windows-task") {
  ...
  if (platform === "systemd") {
    return runCommand(
      action === "status"
        ? ["systemctl", "--user", "--no-pager", "status", "roamgate.service"]
...
export function runServiceCommand(
  ...
  try {
    ...
    if (["install", "restart", "reload"].includes(command.action)) {
